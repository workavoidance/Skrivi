...
namespace Reader.Windows
{
    internal sealed class VoxEngine : ISpeechEngine
    {
        ...
        {
            value.Validate();
            if (settings.Steps != value.Steps || settings.Guidance != value.Guidance) Stop();
            settings = value;
        }
        ...
        public VoxEngine(string modelPath) { model = modelPath; voices = Path.Combine(Path.GetDirectoryName(Path.GetDirectoryName(model)), "voices"); Directory.CreateDirectory(voices); }
        ...
        async Task EnsureReady(CancellationToken token)
        {
            ...
            var psi = new ProcessStartInfo(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "runtime", "crispasr.exe"))
            {
                Arguments = "--server --backend voxcpm2-tts -m \"" + model + "\" --voice-dir \"" + voices + "\" --host 127.0.0.1 --port " + port + " --ws-port -1 --no-gpu -t " + Math.Max(1, Math.Min(8, Environment.ProcessorCount / 2)),
        ...
        public async Task<byte[]> SynthesizeAsync(string text, CancellationToken token)
        {
            ...
            try
            {
                ...
                Status?.Invoke("Generating speech... Ctrl+Alt+R stops");
                var watch = Stopwatch.StartNew();
